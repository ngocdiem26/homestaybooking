    ...
    <div className="space-y-5 pb-2">
      <section className="rounded-3xl border border-gray-100 bg-[#FAF8F3] p-5 shadow-sm">
        ...
        <div className="mt-5 grid gap-3 md:grid-cols-4">
          <label className="rounded-2xl bg-white p-3 shadow-sm">
            ...
            <input required min={todayString()} type="date" value={booking.checkInDate} onChange={(event) => onBookingChange('checkInDate', event.target.value)} className="mt-2 w-full bg-transparent text-sm font-black text-[#2C1E15] outline-none" />
          ...
          <label className="rounded-2xl bg-white p-3 shadow-sm">
            ...
            <input required min={booking.checkInDate || todayString()} type="date" value={booking.checkOutDate} onChange={(event) => onBookingChange('checkOutDate', event.target.value)} className="mt-2 w-full bg-transparent text-sm font-black text-[#2C1E15] outline-none" />
          ...
          <div className="rounded-2xl bg-white p-3 shadow-sm">
            ...
            <div className="mt-2 flex items-center justify-between rounded-xl bg-[#F4F1EA] p-1">
              ...
              <input value={booking.numberOfGuest} min={1} max={maxGuests} onChange={(event) => updateGuests(event.target.value)} className="w-12 bg-transparent text-center text-sm font-black outline-none" inputMode="numeric" type="number" />
        ...
        {availabilityWarning && <div className="mt-3 rounded-2xl border border-red-100 bg-red-50 px-4 py-3 text-xs font-black leading-5 text-red-700">Cảnh báo: {availabilityWarning}</div>}
      </section>
      ...
      <section className="rounded-3xl border border-gray-100 bg-white p-5 shadow-sm">
        ...
        <div className="mt-5 grid gap-4 md:grid-cols-2">
          <label className="space-y-1 text-sm font-bold text-gray-500">Họ tên <input required minLength={2} autoComplete="name" value={form.customerName} onChange={(e) => onChange('customerName', e.target.value)} className="h-12 w-full rounded-2xl border border-gray-200 px-4 font-semibold text-[#2C1E15] outline-none focus:border-[#2C3E2B]" placeholder="Nguyễn Văn A" /></label>
          <label className="space-y-1 text-sm font-bold text-gray-500">Email <input required type="email" autoComplete="email" value={form.customerEmail} onChange={(e) => onChange('customerEmail', e.target.value)} className="h-12 w-full rounded-2xl border border-gray-200 px-4 font-semibold text-[#2C1E15] outline-none focus:border-[#2C3E2B]" placeholder="email@gmail.com" /></label>
          <label className="space-y-1 text-sm font-bold text-gray-500">Số điện thoại <input required type="tel" inputMode="tel" pattern="^(0|\+84)(3|5|7|8|9)\d{8}$" autoComplete="tel" value={form.customerPhone} onChange={(e) => onChange('customerPhone', e.target.value)} className="h-12 w-full rounded-2xl border border-gray-200 px-4 font-semibold text-[#2C1E15] outline-none focus:border-[#2C3E2B]" placeholder="09xxxxxxxx" /></label>
          <label className="space-y-1 text-sm font-bold text-gray-500 md:col-span-2">Ghi chú cho host <textarea maxLength={500} value={form.note} onChange={(e) => onChange('note', e.target.value)} rows={4} className="w-full resize-none rounded-2xl border border-gray-200 px-4 py-3 font-semibold text-[#2C1E15] outline-none focus:border-[#2C3E2B]" placeholder="Ví dụ: Tôi muốn nhận phòng sớm hơn 30 phút..." /></label>
        </div>
      </section>
    </div>
  );
