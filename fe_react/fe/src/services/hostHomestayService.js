...
function normalizeBackendUrl(url) {
  ...
  // Ảnh đặt trong frontend/public/images phải giữ nguyên đường dẫn.
  // Ví dụ: /images/homestay/H1/chinh.jpg
  if (normalizedUrl.startsWith('/images/')) {
    return normalizedUrl;
  }

  // Ảnh upload từ backend vẫn dùng API backend như cũ.
...
function normalizeMainImageFlags(images = []) {
  if (!images.length) return [];

  const selectedIndex = images.findIndex((image) => Boolean(image.isMain));
...
export function mapApiHomestay(apiHomestay) {
  return {
    ...
    beds: Number(apiHomestay.bedCount || 1),
    checkinTime: normalizeTime(apiHomestay.checkinTime, '14:00'),
    checkinEndTime: normalizeTime(apiHomestay.checkinEndTime, '20:00'),
    checkoutStartTime: normalizeTime(apiHomestay.checkoutStartTime, '08:00'),
    checkoutTime: normalizeTime(apiHomestay.checkoutTime, '12:00'),
    latitude: apiHomestay.latitude ?? '',
    longitude: apiHomestay.longitude ?? '',
    ratingAvg: Number(apiHomestay.ratingAvg || 0),
    ...
    createdAt: apiHomestay.createdAt ? String(apiHomestay.createdAt).slice(0, 10) : '',
    updatedAt: apiHomestay.updatedAt ? String(apiHomestay.updatedAt).slice(0, 10) : '',
    images: normalizeMainImageFlags((apiHomestay.images || []).map((image, index) => ({
      imageId: image.imageId,
      url: normalizeBackendUrl(image.imageUrl),
    ...
    services: (apiHomestay.services || []).map((service) => ({
      ...
      description: service.description || '',
      price: Number(service.price || 0),
      pricingUnit: service.pricingUnit || service.pricing_unit || 'PER_DAY',
      status: toUiServiceStatus(service.status),
    })),
    rules: apiHomestay.rules || [],
  };
...
export function mapUiHomestayToRequest(homestay, extraConfig = {}) {
  ...
  return {
    ...
    bedCount: Number(homestay.beds || 1),
    checkinTime: homestay.checkinTime || '14:00',
    checkinEndTime: homestay.checkinEndTime || '20:00',
    checkoutStartTime: homestay.checkoutStartTime || '08:00',
    checkoutTime: homestay.checkoutTime || '12:00',
    latitude: homestay.latitude === '' ? null : homestay.latitude,
    longitude: homestay.longitude === '' ? null : homestay.longitude,
    images: normalizeMainImageFlags(
      images.filter((image) => image?.url && !String(image.url).startsWith('blob:'))
    ).map((image, index) => ({
      imageId: image.imageId,
      imageUrl: image.url,
      isMain: Boolean(image.isMain),
