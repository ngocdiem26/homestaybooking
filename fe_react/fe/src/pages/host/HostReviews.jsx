  ...
  return (
    <HostLayout>
      <div className="space-y-5 text-left text-sm">
        <button type="button" onClick={() => navigate('/host')} className="inline-flex items-center justify-center gap-1.5 rounded-xl border border-gray-200/60 bg-white px-3.5 py-2 text-xs font-bold text-gray-600 shadow-sm transition hover:bg-gray-100">&lt; Về bảng điều khiển</button>
        ...
        <section className="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm">
          <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            <div className="flex flex-wrap gap-2">
              {['ALL', 'WAITING', 'REPLIED'].map((value) => <button key={value} type="button" onClick={() => { setStatusFilter(value); setCurrentPage(1); }} className={'rounded-xl px-4 py-2 text-xs font-black ' + (statusFilter === value ? 'bg-[#2C3E2B] text-white' : 'bg-gray-50 text-gray-500')}>{value === 'ALL' ? 'Tất cả' : value === 'WAITING' ? 'Chưa phản hồi' : 'Đã phản hồi'}</button>)}
            </div>
            <div className="relative w-full lg:w-[420px]"><HiMagnifyingGlass className="absolute left-4 top-1/2 h-5 w-5 -translate-y-1/2 text-gray-400" /><input value={searchTerm} onChange={(event) => setSearchTerm(event.target.value)} placeholder="Tìm khách, homestay, mã đơn..." className="h-12 w-full rounded-xl border border-gray-200 pl-11 pr-4 text-sm font-semibold outline-none focus:border-[#2C3E2B]" /></div>
          </div>
        </section>
