...
//   // Xử lý gửi phản hồi / đổi trạng thái sang đã xử lý
//   const handleSendReply = (e) => {
//     e.preventDefault();
//     if (!replyContent.trim()) return alert("Vui lòng nhập nội dung phản hồi!");
    
//     setComplaints(complaints.map(c => 
//       c.id === activeComplaint.id ? { ...c, status: "Đã xử lý" } : c
//     ));
...
//             <div className="w-11 h-11 bg-[#2C3E2B]/10 rounded-xl flex items-center justify-center text-[#2C3E2B] shrink-0 border border-[#2C3E2B]/5 shadow-inner mt-0.5"><span className="text-xl">💬</span></div>
//             <div>
...
//                 Tiếp nhận, cập nhật trạng thái "Chưa xử lý", "Đang xử lý" hoặc "Đã xử lý" và giải quyết phản hồi từ khách hàng.
//               </p>
//             </div>
//           </div>
//         </div>
...
//           <div className="flex gap-1.5 bg-[#EBE7DF] p-1 rounded-xl font-bold text-[11px] shadow-inner border border-gray-300/30 overflow-x-auto max-w-full shrink-0 h-fit scrollbar-none">
...
//             <button onClick={() => { setStatusFilter('Chờ xác nhận'); setCurrentPage(1); }} className={`px-4 py-1.5 rounded-lg transition-all whitespace-nowrap cursor-pointer ${statusFilter === 'Chờ xác nhận' ? 'bg-[#2C3E2B] text-white shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}>Chưa xử lý</button>
...
//                   <th className="p-4 w-[15%]">Loại</th>
//                   <th className="p-4 w-[13%]">Đơn #</th>
//                   <th className="p-4 w-[20%]">Tiêu đề</th>
//                   <th className="p-4 w-[13%] text-center">Trạng thái</th>
//                   <th className="p-4 w-[13%]">Thời gian</th>
...
//                       <td className="p-4 truncate font-bold">{c.title}</td>
...
//                       {/* BẢNG THAO TÁC RỚT DÒNG NGANG: KÍNH LÚP - MẮT - PHẢN HỒI (Tab trạng thái xổ xuống đặt trong tác vụ) */}
//                       <td className="p-4 text-right whitespace-normal">
//                         <div className="flex items-center justify-end gap-1.5 shrink-0">
                          
//                           {/* Dropdown trạng thái ngay cột thao tác */}
//                           <select 
//                             value={c.status} 
//                             onChange={(e) => handleStatusChange(c.id, e.target.value)}
//                             className="h-8 px-2 bg-gray-50 border border-gray-200 rounded-xl text-[10px] font-bold text-gray-600 focus:outline-none focus:ring-2 focus:ring-[#2C3E2B]/20 cursor-pointer"
//                           >
//                             <option value="Chờ xác nhận">Chưa xử lý</option>
//                             <option value="Đang xử lý">Đang xử lý</option>
//                             <option value="Đã xử lý">Đã xử lý</option>
//                           </select>

//                           {/* 1. NÚT XEM KHIẾU NẠI (Kính lúp bo góc) */}
//                           <button onClick={() => openDetailModal(c)} className="inline-flex items-center justify-center p-2 bg-gray-100 text-gray-600 rounded-xl hover:bg-gray-200 transition border-none cursor-pointer shrink-0" title="Xem chi tiết khiếu nại">
...
//                           {/* 2. NÚT XEM ĐƠN ĐẶT PHÒNG (Con mắt Solid) */}
//                           <button onClick={() => openBookingDetail(c)} className="inline-flex items-center justify-center p-2 bg-blue-50 text-blue-600 rounded-xl hover:bg-blue-100 transition border-none cursor-pointer shrink-0" title="Xem chi tiết đơn đã khiếu nại">
//                             <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-4 h-4"><path d="M12 15a3 3 0 100-6 3 3 0 000 6z"/><path fillRule="evenodd" d="M1.323 11.416C2.405 7.302 6.134 4.35 10.5 4.35s8.094 2.953 9.177 7.066c.11.422.11.866 0 1.288-1.083 4.114-4.81 7.066-9.177 7.066s-8.094-2.952-9.177-7.066a1.921 1.921 0 010-1.288z" clipRule="evenodd"/></svg>
//                           </button>

//                           {/* 3. NÚT PHẢN HỒI (Cây bút) - Disable khi trạng thái là Đã xử lý */}
//                           <button 
//                             onClick={() => openReplyModal(c)} 
//                             disabled={c.status === 'Đã xử lý'}
//                             className={`inline-flex items-center justify-center gap-1 text-xs px-2.5 h-8 rounded-xl font-bold transition duration-200 cursor-pointer border-none shrink-0
//                               ${c.status === 'Đã xử lý' ? 'bg-gray-100 text-gray-400 cursor-not-allowed shadow-none' : 'bg-[#2C3E2B]/10 hover:bg-[#2C3E2B]/20 text-[#2C3E2B] shadow-sm active:scale-95'}`}
//                             title={c.status === 'Đã xử lý' ? 'Đã phản hồi xong' : 'Phản hồi khiếu nại'}
//                           >
//                             <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-3.5 h-3.5"><path d="M21.731 2.269a2.625 2.625 0 00-3.712 0l-1.157 1.157 3.712 3.712 1.157-1.157a2.625 2.625 0 000-3.712z"/></svg>
...
//                 <label className="text-gray-400 text-xs font-bold">Cập nhật trạng thái xử lý:</label>
//                 <select 
//                   value={activeComplaint.status}
//                   onChange={(e) => handleStatusChange(activeComplaint.id, e.target.value)}
//                   className="w-full h-9 px-3 rounded-xl border border-gray-200 bg-white focus:outline-none text-xs font-bold cursor-pointer"
//                 >
//                   <option value="Chờ xác nhận">Chưa xử lý</option>
//                   <option value="Đang xử lý">Đang xử lý</option>
//                   <option value="Đã xử lý">Đã xử lý</option>
