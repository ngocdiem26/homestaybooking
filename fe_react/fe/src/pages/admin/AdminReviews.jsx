...
import { HiCheckCircle, HiEye, HiEyeSlash, HiMagnifyingGlass, HiShieldExclamation, HiStar, HiTrash, HiXMark } from 'react-icons/hi2';
...
export default function AdminReviews() {
  ...
  const filteredReviews = useMemo(() => {
    ...
    return reviews.filter((review) => !keyword || [review.customerName, review.customerEmail, review.homestayName, review.bookingCode, review.comment, review.moderationReason]
  ...
  return (
    <AdminLayout>
      <div className="space-y-5 text-sm">
        <button type="button" onClick={() => navigate('/admin')} className="rounded-xl border border-gray-200 bg-white px-4 py-2 text-xs font-black text-gray-600 shadow-sm hover:bg-gray-50">&lt; Về bảng điều khiển</button>
        ...
        <section className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
          <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            ...
            <button type="button" onClick={() => loadReviews(activeTab)} className="h-11 rounded-xl bg-[#2C3E2B] px-5 text-sm font-black text-white shadow">Làm mới</button>
          </div>
        </section>
          ...
          <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            <div className="flex flex-wrap gap-2">
              {TABS.map(([value, label, key]) => (
                <button key={value} type="button" onClick={() => { setActiveTab(value); setCurrentPage(1); }} className={'rounded-xl px-4 py-2 text-xs font-black transition ' + (activeTab === value ? 'bg-[#2C3E2B] text-white shadow' : 'bg-gray-50 text-gray-500 hover:bg-gray-100')}>
                  {label} ({counts[key] ?? 0})
                </button>
              ))}
            </div>
            <div className="relative w-full lg:w-[420px]">
              ...
              <input value={searchTerm} onChange={(event) => setSearchTerm(event.target.value)} placeholder="Tìm khách, homestay, mã đơn..." className="h-12 w-full rounded-xl border border-gray-200 pl-11 pr-4 text-sm font-semibold outline-none focus:border-[#2C3E2B]" />
            </div>
          </div>
        </section>
