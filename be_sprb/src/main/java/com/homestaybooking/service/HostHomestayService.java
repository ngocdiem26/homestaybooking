...
public class HostHomestayService {
    ...
    private void applyFields(Homestay homestay, HostHomestayRequest request) {
        ...
        homestay.setLivingRoomCount(defaultInt(request.getLivingRoomCount(), 0));
        homestay.setBedCount(defaultInt(request.getBedCount(), 1));
        LocalTime checkinTime = defaultTime(request.getCheckinTime(), LocalTime.of(14, 0));
        LocalTime checkinEndTime = defaultTime(request.getCheckinEndTime(), LocalTime.of(20, 0));
        LocalTime checkoutStartTime = defaultTime(request.getCheckoutStartTime(), LocalTime.of(8, 0));
        LocalTime checkoutTime = defaultTime(request.getCheckoutTime(), LocalTime.of(12, 0));

        if (!checkinEndTime.isAfter(checkinTime)) {
            throw new AppException("Giờ kết thúc nhận phòng phải sau giờ bắt đầu nhận phòng");
        }
        if (!checkoutTime.isAfter(checkoutStartTime)) {
            throw new AppException("Giờ kết thúc trả phòng phải sau giờ bắt đầu trả phòng");
        }

        homestay.setCheckinTime(checkinTime);
        homestay.setCheckinEndTime(checkinEndTime);
        homestay.setCheckoutStartTime(checkoutStartTime);
        homestay.setCheckoutTime(checkoutTime);
    }
    ...
    private void syncServices(Integer homeId, List<HostHomestayServiceRequest> services) {
        ...
        for (HostHomestayServiceRequest service : services) {
            ...
            if (homestayServiceId != null) {
                jdbcTemplate.update(
                        "update homestay_services set service_id = ?, price = ?, pricing_unit = ?, status = ? where homestay_service_id = ? and home_id = ?",
                        serviceId,
                        defaultMoney(service.getPrice()),
                        normalizePricingUnit(service.getPricingUnit()),
                        normalizeServiceStatus(service.getStatus()),
                        homestayServiceId,
                        homeId
                );
            ...
            jdbcTemplate.update(connection -> {
                PreparedStatement statement = connection.prepareStatement(
                        "insert into homestay_services (service_id, home_id, price, pricing_unit, status, created_at) values (?, ?, ?, ?, ?, ?)",
                        Statement.RETURN_GENERATED_KEYS
                );
                statement.setInt(1, serviceId);
                statement.setInt(2, homeId);
                statement.setBigDecimal(3, defaultMoney(service.getPrice()));
                statement.setString(4, normalizePricingUnit(service.getPricingUnit()));
                statement.setString(5, normalizeServiceStatus(service.getStatus()));
    ...
    private List<HostHomestayServiceResponse> getServiceResponses(Integer homeId) {
        return jdbcTemplate.query(
                "select hs.homestay_service_id, s.service_id, s.service_name, s.description, hs.price, hs.pricing_unit, hs.status from homestay_services hs join services s on s.service_id = hs.service_id where hs.home_id = ? order by s.service_name",
                (rs, rowNum) -> HostHomestayServiceResponse.builder()
                        .homestayServiceId(rs.getInt("homestay_service_id"))
                        .serviceId(rs.getInt("service_id"))
                        .serviceName(rs.getString("service_name"))
    ...
    private String normalizePricingUnit(String pricingUnit) {
        if (isBlank(pricingUnit)) return "PER_DAY";
        String normalized = pricingUnit.trim().toUpperCase();
        return "PER_USE".equals(normalized) ? "PER_USE" : "PER_DAY";
    }
